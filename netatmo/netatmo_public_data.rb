...
module Netatmo
  ...
  class Station
    ...
    def initialize(token, lat_ne, lon_ne, lat_sw, lon_sw)
      @token = token
      @id = id
      @lat_ne = lat_ne
      @lon_ne = lon_ne
    ...
    def update_data
      uri = URI(@@url)
      params = { access_token: @token, lat_ne: @lat_ne, lon_ne: @lon_ne, lat_sw: @lat_sw, lon_sw: @lon_sw }
  ...
  class App
    ...
    def station
      Netatmo::Station.new(token, config.lat_ne, config.lon_ne, config.lat_sw, config.lon_sw)
    end
  end
